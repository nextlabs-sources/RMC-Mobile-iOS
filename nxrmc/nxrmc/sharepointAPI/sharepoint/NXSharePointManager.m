...
-(void) initRemoteQueryResponseHandler
{
    ...
    RemoteQueryResponseHandler downloadFile = ^(NXSharePointRemoteQueryBase* query, NSData* data)
    {
        ...
        // 3. notify delegate
        __strong NXSharePointManager *strongself = weakself;
        if (DELEGATE_HAS_METHOD(strongself.delegate, @selector(didDownloadFile:fileData:))) {
            [strongself.delegate didDownloadFile:fileName fileData:data];
        }
    };
...
- (NXSharePointRemoteQueryBase*) initializeQuery:(NSString *)queryURLStr {
    ...
    switch (_spMgrType) {
        ...
        case kSPMgrSharePointOnline:
            spQuery = [[NXSharepointOnlineRemoteQuery alloc] initWithURL:queryURLStr repoId:self.repoId];
            break;
        default:
            break;
    }
...
{
    NSString* queryDownloadFile;
    
    if (self.spMgrType == kSPMgrSharePoint) {
          queryDownloadFile = [NSString stringWithFormat:[NXSharePointRestTemp SPDownloadSharePointFileTemp], self.siteURL, fileUrl];
    }else{
        queryDownloadFile = [NSString stringWithFormat:[NXSharePointRestTemp SPDownloadFileTemp], self.siteURL, fileUrl];
    }
