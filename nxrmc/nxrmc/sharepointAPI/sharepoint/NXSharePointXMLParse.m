...
+(NSDictionary *) parseSiteQuota:(NSData *) data
{
    ...
    NXXMLDocument *xmlDoc = [NXXMLDocument documentWithData:data error:nil];
    NXXMLElement *root = xmlDoc.root;
    NXXMLElement *siteStorageUsed = [root childNamed:SP_STORAGE_TAG];
    ...
    if (siteStorageUsed.value) {
        NSNumber *storage = [numFormat numberFromString:siteStorageUsed.value];
        [retDict setObject:storage forKey:SP_STORAGE_USED_TAG];
        
        if (siteStoragePercentageUsed.value) {
            NSNumber *percentageUsed = [numFormat numberFromString:siteStoragePercentageUsed.value];
            if ( percentageUsed.doubleValue <= 0) {
                percentageUsed = [NSNumber numberWithInt:1.0];
            }
            long totalStorage = storage.doubleValue / percentageUsed.doubleValue;
            NSNumber *totalStorageNum = [NSNumber numberWithLong:totalStorage];
            [retDict setObject:totalStorageNum forKey:SP_STORAGE_TAG];
        }
    }
    return retDict;
}
