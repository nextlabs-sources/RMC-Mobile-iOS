    ...
    // 非空dayView的状态
        ...
        if (_manager.selectionType == ZYCalendarSelectionTypeSingle) {
            if (_manager.selectedDateArray.count && [_manager.helper date:_date isTheSameDayThan:_manager.selectedDateArray.firstObject]) {
                ...
                if ([_manager.helper date:_date isEqualOrAfter:_manager.date] && [_manager.helper date:_date isEqualOrBefore:_manager.selectedDateArray.firstObject]) {
                      self.selected = true;
                }
                else
                {
        ...
        // 范围选择
            ...
            if (_manager.selectedDateArray.count == 2) {
                ...
                if (!_manager.canSelectPastDays &&
                    ...
                    if ([_manager.helper date:_date
                                      isAfter:_manager.selectedDateArray[0]
                                    andBefore:_manager.selectedDateArray[1]] || [_manager.helper date:_date isTheSameDayThan:_manager.selectedDateArray[0]] || [_manager.helper date:_date isTheSameDayThan:_manager.selectedDateArray[1]]) {
                        
                         [self setImage:self.getDisabledImage forState:UIControlStateDisabled];
                    }
                    else
                    {
                         self.selected = false;
...
-(UIImage *)getDisabledImage
{
    if (!_disabledImage) {
        _disabledImage = [self imageByApplyingAlpha:0.45 image:[[UIImage imageNamed:@"circle"] imageWithRenderingMode:_manager.imageRenderingMode]];
    }
