...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = NSLocalizedString(@"UI_ACCOUNT_INFO", NULL);
    NSString* userName = [NXLoginUser sharedInstance].profile.userName;
    if (!userName || userName.length == 0) {
        userName = @"";
    }
    _dataArray = @[@{NSLocalizedString(@"UI_ACCOUNT_INFO_NAME", NULL) : userName},
                   @{NSLocalizedString(@"UI_ACCOUNT_INFO_EMAIL", NULL) : [NXLoginUser sharedInstance].profile.email},
                   @{NSLocalizedString(@"UI_ACCOUNT_INFO_TENANT_ID", NULL) : [NXCommonUtils currentTenant]}];
