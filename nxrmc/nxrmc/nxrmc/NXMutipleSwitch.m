...
@interface NXMultipleSwitchLayer : CALayer

@end

@implementation NXMultipleSwitchLayer
- (instancetype)init {
    if (self = [super init]) {
...
+ (Class)layerClass{
    return [NXMultipleSwitchLayer class];
}

@end

@interface NXMutipleSwitch()
...
@implementation NXMutipleSwitch
...
- (void)layoutSubviews {
    ...
    self.tracker.frame = CGRectMake(_selectedSegmentIndex * averageWidth, 2, averageWidth - 2, contentRect.size.height - 4);
