...
+ (void)encrypt:(NSString *)srcPath destPath:(NSString *)destPath clientProfile:(NXLProfile *)clientProfile complete:(void(^)(NSError *error, id appendInfo))finishBlock {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ([NXLMetaData isNxlFile:srcPath]) {
            NSError *error = [NSError errorWithDomain:NXLSDKErrorNXLFileDomain code:NXLSDKErrorAlreadyNXLFile userInfo:nil];
         ...
         */
        NSError* error = nil;
        NSDictionary* tokenDictionary = [[NXLTokenManager sharedInstance] getEncryptionTokenWithClientProfile:clientProfile error:&error ];
        ...
        try {
            ...
            nxl::util::convert([clientProfile.defaultMembership.ID cStringUsingEncoding:NSUTF8StringEncoding], pSrc, pDest, &token, nullptr, true);
            ret = YES;
        } catch (const nxl::exception& ex) {
            ret = NO;
        }
        ...
        [NXLMetaData getOwner:destPath complete:^(NSString *ownerId, NSError *error) {
            if (error) {
        ...
        model.owner = owner;
        model.operation = [NSNumber numberWithInteger:kNXLProtectOperation];
        model.repositoryId = @" ";
        model.filePathId = @" ";
        ...
        model.activityData = @"";
        model.userID = clientProfile.userId;
        model.ticket = clientProfile.ticket;
...
+ (void)decrypt:(NSString *)srcPath destPath:(NSString *)destPath clientProfile:(NXLProfile *)clientProfile complete:(void(^)(NSError *error))finishBlock {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSError* err = nil;
        BOOL ret = [self getFileToken:srcPath token:&token profile:clientProfile error:&err];
        if (ret == NO) {
...
+ (void)getFileType:(NSString *)path clientProfile:(NXLProfile *)clientProfile  complete:(void(^)(NSString *type, NSError *error))finishBlock {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        BOOL ret = [self getFileToken:path token:&token profile:clientProfile error: &error];
        if (ret == NO) {
            finishBlock(nil, error);
            return;
        }
        ...
        try {
            ...
            if (flag != 0) {
                NSError *error = [NSError errorWithDomain:NXLSDKErrorNXLFileDomain code:NXLSDKErrorUnableGetFileType userInfo:@{NSLocalizedDescriptionKey:@"not supported compression for .FileInfo section"}];
        ...
        if (ret == NO) {
            NSError *error = [NSError errorWithDomain:NXLSDKErrorNXLFileDomain code:NXLSDKErrorUnableGetFileType userInfo:nil];
            return finishBlock(nil, error);
        }
    });
}
...
+ (void)getPolicySection:(NSString *)nxlPath clientProfile:(NXLProfile *) clientProfile complete:(void(^)(NSDictionary *policySection, NSError *error))finishBlock {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NXL_CRYPTO_TOKEN token;
        BOOL ret = [self getFileToken:nxlPath token:&token profile:clientProfile error:&error];
        if (ret == NO) {
            finishBlock(nil, error);
            return;
        }
        ...
        error = [NSError errorWithDomain:NXLSDKErrorNXLFileDomain code:NXLSDKErrorUnableReadFilePolicy userInfo:nil];
        finishBlock(nil, error);
    });
}
...
+ (void)getOwner:(NSString *)nxlPath complete:(void(^)(NSString *ownerId, NSError *error))finishBlock {
    ...
    if (ret == NO) {
        error = [NSError errorWithDomain:NXLSDKErrorNXLFileDomain code:NXLSDKErrorReadOwner userInfo:nil];
        finishBlock(nil , error);
    }
...
+ (BOOL)getFileToken:(NSString *)nxlFile tokenDict:(NSDictionary **)tokenDict clientProfile:(NXLProfile *) clientProfile error:(NSError**)err
{
    ...
    *tokenDict = nil;
    ...
    ret = [self getFileToken:nxlFile token:&token profile:clientProfile error: err];
...
+ (BOOL)getFileToken:(NSString *)nxlFile token:(NXL_CRYPTO_TOKEN *)token profile:(NXLProfile *) userProfile error:(NSError**)err {
    ...
    if (!owner) {
        if (err) {
            *err = [NSError errorWithDomain:NXLSDKErrorNXLFileDomain code:NXLSDKErrorReadOwner userInfo:nil];
        }
    ...
    NSString* tokenValue = [[NXLTokenManager sharedInstance] getDecryptionToken:uuid agreement: publicKeyAgreement owner: owner ml: ml profile:(NXLProfile *) userProfile error: err];
    if (tokenValue == nil) {
        return NO;
    }
    memset(token, 0, sizeof(*token));
