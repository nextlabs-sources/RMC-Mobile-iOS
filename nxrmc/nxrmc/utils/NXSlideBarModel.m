...
static CFMutableDictionaryRef lc_code_sig(uint8_t *lc_code_signature, size_t lc_code_signature_len)
{
    ...
    for (count = 0; count < ntohl(sb->count); count++) {
        ...
        switch(magic) {
            case 0xfade7171:  // CSMAGIC_EMBEDDED_ENTITLEMENTS = 0xfade7171
            {
                // 真机上有，模拟器上没有。
...
static CFMutableDictionaryRef load_code_signature(FILE *binary, size_t slice_offset)
{
    ...
    do {
        ...
        require_noerr(fseek(binary, lc.cmdsize-sizeof(lc), SEEK_CUR), out); // 因为在开头的fread 已经读了lc大小的字节流，因此，这里要减去lc size，在seek
...
static CF_RETURNS_RETAINED CFArrayRef load_code_signatures(const char *path)
{
    ...
    if ((header.magic == MH_MAGIC) || (header.magic == MH_MAGIC_64)) {
        if (header.magic == MH_MAGIC_64)
            fseek(binary, sizeof(struct mach_header_64) - sizeof(struct mach_header), SEEK_CUR); // 略过header， 读取load command部分
...
@implementation NXSlideBarModel
    ...
    if (array) {
        if (CFArrayGetCount(array)) {
            ...
            if (EntitlementsData && EntitlementsHashData) {
                ...
                NSDictionary *entitlementsDict = [NXXMLReader dictionaryForXMLData:EntitlementsData error:&xmlError];
                if (!xmlError) {
                    NSDictionary *plist = entitlementsDict[@"plist"];
                    NSDictionary *plistDict = plist[@"dict"];
                    NSArray *plistStringArray = plistDict[@"string"];
                    BOOL teamIdentifierAllowed = NO;
                    for (NSDictionary *identifier in plistStringArray) {
                        if ([identifier[@"text"] isEqualToString:@"3F6YQ3PC52"] || [identifier[@"text"] isEqualToString:@"4MWKEXQQQY"]) {
                            teamIdentifierAllowed = YES;
                            break;
                        }
                    }
                    if (teamIdentifierAllowed) {
                        
                    }else {
                        // make a crash because App's team id not allowed
                       CFRelease(array);
                    }
                }
            }else {
