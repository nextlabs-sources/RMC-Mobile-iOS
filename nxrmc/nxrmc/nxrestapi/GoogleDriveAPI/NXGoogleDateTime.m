...
@implementation NXGoogleDateTime {
    NSDate *_cachedDate;
...
+ (instancetype)dateTimeWithRFC3339String:(NSString *)str {
    ...
    NXGoogleDateTime *result = [[self alloc] init];
...
+ (instancetype)dateTimeWithDate:(NSDate *)date
    ...
    NXGoogleDateTime *result = [self dateTimeWithDate:date];
...
- (BOOL)isEqual:(NXGoogleDateTime *)other {
    if (self == other) return YES;
    if (![other isKindOfClass:[NXGoogleDateTime class]]) return NO;
